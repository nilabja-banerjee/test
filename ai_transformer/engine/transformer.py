...
class CodeTransformer:
    ...
    def call_task_model(self, task_name: str, context_vars: dict, context_id: str) -> str:
        ...
        return self.model_router.route(task_name, messages)
    ...
    def transform_file(self, class_name, file_meta, file_content):
        ...
        coverage, issues = check_coverage(
            ...
            reviewer_model="review_coverage",  # new task name
        ...
        while coverage < 99 and iteration <= 3:
            ...
            feedback_vars = {
                "source_code": file_content,
                "initial_generated_code": transformed_code,
                "review_feedback": "\n".join(issues),
                "context_id": context_id
            }

            feedback_response = self.call_task_model("apply_feedback", feedback_vars, context_id)
            ...
            coverage, issues = check_coverage(
                source_code=file_content,
                generated_code=transformed_code,
                reviewer_model="review_coverage",
                llm_settings=self.llm_settings,
                prompt_dir=self.prompt_dir,
                context_id=context_id,
                model_router=self.model_router
